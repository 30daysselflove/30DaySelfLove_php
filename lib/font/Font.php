...
class Font {
   ...
   */
  public static function load($file, $isStream = false) {
    if($isStream) $header = stream_get_contents($file, 4);
    else $header = file_get_contents($file, false, null, null, 4);
    $class = null;
    ...
    switch($header) {
      ...
      // Unknown type or EOT
      default:
        if($isStream) $magicNumber = stream_get_contents($file, 2, 34);
        else $magicNumber = file_get_contents($file, false, null, 34, 2);
