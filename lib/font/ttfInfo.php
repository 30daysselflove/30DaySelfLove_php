...
class ttfInfo {
    ...
    /**
     * function setFontStream()
     * set the filename
     * @public
     * @param string $data the new value
     * @return object reference to this
     */
    public function setFontStream($data)
    {
        $this->_stream = $data;
        return $this;
    ...
    public function getFontInfo()
    {
        ...
        if($this->_stream) $this->text = stream_get_contents($this->_stream);
        else
        {
        ...
        for ($j=0;$j<$number_name_records_dec;$j++)
        {
            ...
            if (!empty($name_id_dec) and empty($font_tags[$name_id_dec]))
            {
                for($l=0;$l<$string_length_dec;$l++)
                {
                    ...
                    elseif(isset($this->text[$storage_dec+$string_offset_dec+$l])) {
                        if(isset($font_tags[$name_id_dec])) $font_tags[$name_id_dec] .= ($this->text[$storage_dec+$string_offset_dec+$l]);
