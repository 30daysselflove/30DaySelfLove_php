 ...
 */
use core\memcache\CacheCentralStation;
...
class LightOpenID
{
    ...
    function __construct($host)
    {
        ...
        //echo "exists:" . (bool) function_exists('curl_init') == true;
    ...
    protected function papeParams()
    {
        ...
        if ($this->pape)
        {
            ...
            foreach ($this->pape as $papeKey => $papeVal)
            {
                $namespaceKey = "openid.pape." . $papeKey;
                $params[$namespaceKey] = $papeVal;
            }
        }
        return $params;
    }
    ...
    function authUrl($immediate = false)
    {
        ...
        /* @var $cache MemcachedStore */
    	$cache = CacheCentralStation::getCacheByConnectionID();
    	$cacheKey = __NAMESPACE__ . "_" . __CLASS__ . "_dis_" . $this->identity;
    	...
    	if(is_array($result))
    	{
    		...
    		$data = $result[0];
    		if(isset($data['ax'])) 		$this->ax  = $data['ax'];
    		if(isset($data['version'])) $this->version  = $data['version'];
    		if(isset($data['sreg'])) $this->sreg  = $data['sreg'];
    		if(isset($data['server'])) $this->server  = $data['server'];
        ...
        if (!$this->server) 
        {
        	...
        	$dataArray = array("ax" => $this->ax, "version" => $this->version, "sreg" => $this->sreg, "server" =>$this->server);
        	$cache->set($cacheKey, $dataArray, 604800); // 1 week
        }
