...
class TwitterOAuth {
   ...
   * @returns a string
   */
  function getAuthorizeURL($token, $sign_in_with_twitter = TRUE, $forceLogin = false) {
    if (is_array($token)) {
      $token = $token['oauth_token'];
    }
    if (empty($sign_in_with_twitter)) {
        $forceLoginString = $forceLogin ? "&force_login=true" : "";
      return $this->authorizeURL() . "?oauth_token={$token}" . $forceLoginString;
    } else {
        $forceLoginString = $forceLogin ? "&force_login=true" : "";
       return $this->authenticateURL() . "?oauth_token={$token}" . $forceLoginString;
    }
  }
