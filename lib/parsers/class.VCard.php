...
class VCard
{
    ...
    {
        return isset($this->_map[$name]) ? $this->_map[$name][0] : '';
    }
    ...
    function parse(&$lines)
    {
        while (list(, $line) = each($lines)) {
            ...
            if (count($tmp) == 2) {
                ...
                if (isset($this->params['ENCODING']) && $this->params['ENCODING'][0] == 'QUOTED-PRINTABLE') {
                    $this->_decodeQuotedPrintable($lines);
                }
                if (isset($this->params['CHARSET']) && $this->params['CHARSET'][0] == 'UTF-8') {
